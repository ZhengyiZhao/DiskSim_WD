...
struct lp_value *lp_new_intv(int64_t i) // @9
{
...
int check_types(struct lp_block *b) {
  ...
  for(c = 0; c < b->params_len; c++) {
	...
	if(((lp_modules[b->type]->modvars[lp_param_name(b->type, b->params[c]->name)].type
	  ...
	  fprintf(stderr, "*** error: type error: %s::\"%s\" cannot take type \n PTYPE(b->params[c]):%d, \n lp_modules[b->type]->modvars[lp_param_name(b->type, b->params[c]->name)].type:%d \nb->type:%d, b->params[c]->name:%s",
		  lp_modules[b->type]->name,
		  b->params[c]->name,
		  PTYPE(b->params[c]),
		  lp_modules[b->type]->modvars[lp_param_name(b->type, b->params[c]->name)].type,
				  b->type,
				  b->params[c]->name); // @9
...
lp_loadparams(void *it, struct lp_block *b, struct lp_mod *m) {
  ...
  FOUND:
    ...
    else
    {
		  ...
		  {
			  case I:
		   // long int  lTemp = (b->params[c])->v->v.i;
			((lp_paramloader_int)m->param_loaders[pnum])(it, (b->params[c])->v->v.i); // IVAL , @9
			break;
			  case D:
			...
			((lp_paramloader_list)m->param_loaders[pnum])(it, LVAL(b->params[c]));
			 break;
			  case L: // @9
			((lp_paramloader_int)m->param_loaders[pnum])(it, (b->params[c])->v->v.i);
			break;
			  case BLOCK:
			  default:
  ...
  for(c = 0; c < m->modvars_len; c++)
  {
    ...
    { // @016 set to
      fprintf(stderr, "*** warning: in %s spec -- missing required parameter %s\n",
