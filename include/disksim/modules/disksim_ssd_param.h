...
struct ssd *disksim_ssd_loadparams(struct lp_block *b, int *num);
...
typedef enum {
   ...
   DISKSIM_SSD_TIMING_MODEL,
   DISKSIM_SSD_FLASH_CHIP_ELEMENTS,
   DISKSIM_SSD_PAGE_SIZE,
   DISKSIM_SSD_PAGES_PER_BLOCK,
   DISKSIM_SSD_BLOCKS_PER_ELEMENT,
   DISKSIM_SSD_ELEMENT_STRIDE_PAGES,
   ...
   DISKSIM_SSD_RESERVE_PAGES_PERCENTAGE,
   DISKSIM_SSD_MINIMUM_FREE_BLOCKS_PERCENTAGE,
   DISKSIM_SSD_CLEANING_POLICY,
   DISKSIM_SSD_PLANES_PER_PACKAGE,
   DISKSIM_SSD_BLOCKS_PER_PLANE,
   DISKSIM_SSD_PLANE_BLOCK_MAPPING,
   DISKSIM_SSD_COPY_BACK,
   DISKSIM_SSD_NUMBER_OF_PARALLEL_UNITS,
   DISKSIM_SSD_ELEMENTS_PER_GANG,
   DISKSIM_SSD_CLEANING_IN_BACKGROUND,
   DISKSIM_SSD_GANG_SHARE,
   DISKSIM_SSD_ALLOCATION_POOL_LOGIC
} disksim_ssd_param_t;

#define DISKSIM_SSD_MAX_PARAM		DISKSIM_SSD_ALLOCATION_POOL_LOGIC
...
#define DISKSIM_SSD_MAX 31
static struct lp_mod disksim_ssd_mod = { "disksim_ssd", disksim_ssd_params, DISKSIM_SSD_MAX, (lp_modloader_t)disksim_ssd_loadparams,  0, 0, DISKSIM_SSD_loaders, DISKSIM_SSD_deps };
