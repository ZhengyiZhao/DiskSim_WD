...
find_band_lbn(struct dm_layout_g1 *l, LBA_TYPE lbn) // @9
{
  struct dm_layout_g1_band *b = &l->bands[0];
  LBA_TYPE bandstart = 0;
  int bandno = 0;
  LBA_TYPE templbn = lbn; // @9
  // @9 <0 ?
  while((templbn >= b->blksinband) || (templbn < 0)) {
//    fprintf( stderr, "\nlbn %15.0f, templbn %15.0f, blksinband:%d, ....", (double)lbn, (double)templbn, b->blksinband);
    bandstart += b->blksinband;
    templbn -= b->blksinband;
    bandno++;
    b = &l->bands[bandno];
//    fprintf( stderr, " templbn %15.0f, num %d, startcyl %d, endcyl %d, blkspertrack %d, blksinband %d, deadspace %d, bands_len %d, bandno %d", (double)templbn, b->num, b->startcyl, b->endcyl, b->blkspertrack, b->blksinband, b->deadspace,  l->bands_len, bandno );
    ddbg_assert3((bandno < l->bands_len),
     ("bandno: %d, l->bands_len: %d, lbn: %15.0f, bandstart:%15.0f, b->blksinband:%d, templbn:%15.0f", bandno, l->bands_len, (double)lbn, (double)bandstart, b->blksinband, (double)templbn)
     );
...
static int 
g1_st_lbn(struct dm_disk_if *d, LBA_TYPE lbn) //@9
{
...

{
  struct dm_layout_g1 *l = (struct dm_layout_g1 *)d->layout;
  struct dm_layout_g1_band *b = find_band_pbn(l, p);
  LBA_TYPE lbn = l->band_blknos[b->num];  // @9
  struct dm_pbn pbn = *p;
			      ...
			      int *remapsector)
{
  ...
  struct dm_layout_g1 *l = (struct dm_layout_g1 *)d->layout;
  struct dm_layout_g1_band *b = find_band_pbn(l, p);
  LBA_TYPE lbn = l->band_blknos[b->num];
  int temp_lbn = lbn;
...
static dm_angle_t
g1_lbn_offset(struct dm_disk_if *d, LBA_TYPE lbn1, LBA_TYPE lbn2)  // @9
{
...
// @012
int g1_get_zone_id_by_lbn(struct dm_disk_if *disk, LBA_TYPE lbn)
{
	int nTotalZones = g1_get_numzones(disk);
...
// @015
int g1_get_pbn_by_abs_cyl(struct dm_disk_if *disk, int64_t i64_cyl, struct dm_smr_hdcyl_g4_pbn *stpOutputG4PBN_SMR)
{
	int iRet = 1;
...
// @019
int
g1_get_zone_meta_data_by_id(struct dm_disk_if *d, int n, struct dm_layout_zone *z)
{
}

int g1_get_zone_meta_data_by_lbn(struct dm_disk_if *disk, LBA_TYPE lbn,
		struct dm_layout_zone *stpZone_meta_data)
{
	return 0; // zz is the Zone-id: from 0
}

static int
g1_slipcount(struct dm_layout_g1 *l,
		LBA_TYPE lbn)   // @9
{
  return 0;
}
...
void *layout_g1_fns[] = {
  ...
  g1_get_numzones,
  g1_get_zone,
  g1_get_zone_id_by_lbn,   // @012
  g1_get_pbn_by_abs_cyl,    // @015
  g1_get_zone_meta_data_by_lbn,  // @019
  g1_slipcount
};
