		   ...
		   struct lp_list *l) 
{
  ...
  printf("%s() lbn %18.0f alen %f off %f\n", __func__, (double)e->lbn,
	 dm_angle_itod(e->alen), dm_angle_itod(e->off)); // @9
	   ...
	   struct lp_list *l)
{
  ...
  for(i = 0; i < r->idx_len; i++) {
    ...
    struct idx_ent *entj;
//    struct idx_ent *bakEnt; // @9
//    union {char cByte[8]; long int i64; double dbl} vTemp;

    loff = 0;
    
    // this is a little sloppy yields an upper bound, not an
    // exact number
#ifdef __DEBUG_64bitLBA__
    if(i == 60)
    {
    	printf("load IDX\n");
    }
	...
	j++,  entj++) //@9 bakEnt = entj,
    {
      LBA_TYPE off; // @9
      long int ldTemp2, ldTemp;
#ifdef __DEBUG_64bitLBA__
      long int  ldTemp3;
      memcpy(&vTemp, &(li->values[loff]->v.strChar[0]), 8);
#endif

//      ldTemp2 = (long int)(li->values[loff]->v.i);
      ldTemp = (long int)(li->values[loff++]->v.i);
      ...
      {
#ifdef __DEBUG_64bitLBA__
    	  entj->lbn = ((long)(1<<32) + (long)ldTemp); // li->values[loff++]->v.i;
    	  ldTemp2 = (0xFFFFFFFFFFFFFFFF - ldTemp); // li->values[loff++]->v.i;
    	  ldTemp3 = (0xFFFFFFFFFFFFFFFF - ldTemp + 1); // li->values[loff++]->v.i;
    	  entj->lbn = ldTemp3; //
#endif
      }
