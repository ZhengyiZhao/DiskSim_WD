...
// @4.1
void 
dm_mech_g1_read_write_extracted_seek_curve(char *filename, 
				      int *cntptr,
				      int **distsptr, 
				      dm_time_t **timesptr,
                                      dm_time_t **WriteTimesptr,
                                      int *iSeekProfileType)
{
   int rv, mat;
   int lineflag = 1;
   ...
   if(pathname) {
	 ...
	 printf("seek file name: %s", filename);
   ...
   WriteSeekTimes = calloc(buflen, sizeof(*WriteSeekTimes)); // @4.1

   *iSeekProfileType = 0;  // @4.1
   
   do {
     double time, stdev;
     int dist;
     double dblWriteSeekTime;  // @4.1
     ...
     if(rv) {
       mat = sscanf(linebuf, "%d, %lf, %lf, %lf\n", &dist, &time, &stdev,
    		   &dblWriteSeekTime); // @4.1
       
       if(mat == 2 || mat == 3 || mat ==  4) {
	 if(count >= buflen-1) {
	   buflen *= 2;
	   dists = realloc(dists, buflen * sizeof(*dists));
	   times = realloc(times, buflen * sizeof(*times));
           WriteSeekTimes = realloc(WriteSeekTimes, buflen * sizeof(*WriteSeekTimes)); // @4.1
	 }
	 ...
	 times[count] = dm_time_dtoi(time);
         if(mat ==  4)  // @4.1
         {
             WriteSeekTimes[count] = dm_time_dtoi(dblWriteSeekTime);
         }
         else
         {
        	 WriteSeekTimes[count] = 0;
         }   // @4.1
	 count++;
       }
       else {
   ...
   *distsptr = dists;
   *timesptr = times;
   *WriteTimesptr = WriteSeekTimes; // @4.1
}
