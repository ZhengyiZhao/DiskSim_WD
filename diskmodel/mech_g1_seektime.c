	...
	// @4.1
	 ...
	 d2 = m->xseekdists[i];
    	   ...
    	   else
    	   {
    		   ...
    		   {
    				 t1 = m->xwriteseektimes[i-1];
    				 t2 = m->xwriteseektimes[i];
    				 d1 = m->xseekdists[i-1];
    				 d2 = m->xseekdists[i];
    		   }
    	   }
	 ...
	 if(t1 > t2) {
	   // result -= ddiff * (t1 - t2);
	   result -= 1.0 * (dist - d1) * (t1 - t2) / (d2 - d1);
	 }
	 else {
	   //	     result += ddiff * (t2 - t1);
	   result += 1.0 * (dist - d1) * (t2 - t1) / (d2 - d1);
	 }
	 break;
       }
     }
