...
// @011, recalculate the initial-latency, xfertime,
// computes the access time for an access time on a single track
static dm_time_t
dm_acctime_track_immed(struct dm_disk_if *d,
	      struct dm_mech_state *istate,
	      struct dm_pbn *start,
	      int len,
	      int rw,
	      LBA_TYPE nAccessStartLBA,  LBA_TYPE nAccessEndLBA, LBA_TYPE nTrackLastLBA,
	      struct dm_mech_state *result_state,
	      struct dm_mech_acctimes *breakdown)
{
  //  struct dm_mech_state start_track;
  ...
  // calculate the initial LBA after seek
  LBA_TYPE nInitialLBA = d->layout->dm_translate_ptol(d, &state, 0);

  // initial rotational latency given our new position
    atlp = &addtolatency;
  if(nInitialLBA > nAccessStartLBA && nInitialLBA < nAccessEndLBA)
  {
	  latency = d->mech->dm_latency(d,
					&state,
					start->sector,
  ...
  // DONOT add if zero-latency access, add intermediate rotational latency
  latency += immed ? *atlp : 0;
  ...
  // little gross; need to break down the xfertime for zero-latency access
  // @011,
  // This time should be added into the additional latency, BUT not xfertime
	      ...
	      struct dm_mech_acctimes *breakdown)
{
  ...
  LBA_TYPE lbnhigh; // highest lbn on the current track //@9

  LBA_TYPE l;       // first lbn to read  @9
  LBA_TYPE lastlbn; // last lbn to read, // @9
  ...
  do {
    ...
    // dm_acctime_track, @011
    if(immed)
    {
    	result += dm_acctime_track_immed(d,
			       &s, 
			       &pbn, 
			       lbnhigh - l + 1, 
			       rw,
...
// #define DEBUG
#ifdef DEBUG
    printf("(%e, %e, %e), %e, <%d, %d, %d>, %20.0f\n",
  		  dm_angle_itod(start), dm_angle_itod(end), dm_angle_itod(theta),
  		  dm_time_itod(result), begin->head, begin->cyl, (double)start_sect,
  		  result); // dm_angle_itod(extra), , m->rotatetime, calctime
#endif
    return result;
  }
  else {
