...
typedef int LBA_TYPE;
#else  // CONFIG_X86_32

typedef int64_t LBA_TYPE;
#endif
...
// @015, Zhengyi.zhao@wdc.com
struct dm_smr_hdcyl_g4_pbn {
	int nZoneId;
	int nIdxPatternIDX;
	int nIdxPatternTK;
	int nClusterOffset;
...
struct dm_layout_zone {
  ...
  int nByte_per_sector; // @019
  int nSector_per_track_od;
  int nSector_per_track_id;
  int nTrack_per_cluster_od;
  int nTrack_per_cluster_id;
  int nCluster_per_zone;
  int nFlag_is_smr;   // @019

  int nNum_sectors_WRRO;
};
...
struct dm_layout_if {
  ...
  // Return the zone-id, which contains the given LBN // @012
  int (*dm_get_zone_id_by_lbn)(struct dm_disk_if *d, LBA_TYPE lbn);

  // The abs_cyl is the cylinder mapping among all heads, this function is for SMR, // @015
  int (*dm_get_pbn_by_abs_cyl)(struct dm_disk_if *d, int64_t i64_cyl, struct dm_smr_hdcyl_g4_pbn *stpOutputG4PBN_SMR);

  // To return the zone-meta-data, given an LBN @019
  int (*dm_get_zone_meta_data_by_lbn)(struct dm_disk_if *disk, LBA_TYPE lbn, struct dm_layout_zone *stpZone_meta_data);

  //
  int (*dm_get_slip_count_bef_lbn)(struct dm_disk_if *d, LBA_TYPE lbn);  // @015
};
...
struct dm_power_if {
	  ...
	  double (*dm_spindle_power)(struct dm_disk_if *,
			   int nZoneId);
	  // how much energy is applied on the Voice coil to seek from the first to the second track,
	  // This is only track-to-track so the angles in the parameters are
	  // ignored.
	  double (*dm_vcm_seek_energy)(struct dm_disk_if *,
				   struct dm_mech_state *start_track,
				   struct dm_mech_state *end_track,
				   int rw);
...
#include "dm_power.h"  // @011

#define DEF_MEDIA_HD_CMR_ONLY   0
#define DEF_MEDIA_HD_CMR_BIT    1
#define DEF_MEDIA_HD_SMR_BIT    2
#define DEF_MEDIA_HD_CMR_SMR_COMBO   3
...
struct dm_disk_if {
  ...
  int dm_surfaces;
  LBA_TYPE dm_sectors; // @9

  LBA_TYPE iFlag_cmr_smr_combo; // @019
  char cFlag_is_smr;
  char cFlag_is_cmr;

  int nTotal_wedges_per_track;  // @019

  struct dm_layout_if   *layout;
  struct dm_mech_if     *mech;

  struct dm_power_if   *stpPowerHDD;  // @011  HDD_POWER


};


//// Integrate here for the model_calculator
#define CALC_OK  0
#define CALC_ERR 1

/// Error number for the hdd_model_calculator
#define HDD_MODEL_CALCULATOR_MORE_THAN_2_TRACK   (DM_NX - 1)

#define DISKSIM_FLAG_BIT_RW 1
...
typedef struct
{
	...
	double tRegLastEndRW;
}INPUT_HDD_CALCULATE_ACCESS_TIME;
...
}OUTPUT_HDD_CALCULATE_ACCESS_TIME;
...
typedef struct dm_power_if *(*dm_power_loader_t)(struct lp_block *, struct dm_disk_if *); // 011
